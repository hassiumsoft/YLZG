...
/** 初始化友盟统计模块
  UMAnalconfigure类，具体参照该类成员的参数定义
...
/** 设置app版本号。由于历史原因需要和xcode3工程兼容,友盟提取的是Build号(CFBundleVersion)，
   ...
   appVersion 版本号，例如设置成`XcodeAppVersion`.
  
*/
+ (void)setAppVersion:(NSString *)appVersion;

/** 开启CrashReport收集, 默认YES(开启状态).
        value 设置为NO,可关闭友盟CrashReport收集功能.
  
*/
+ (void)setCrashReportEnabled:(BOOL)value;

/** 设置是否打印sdk的log信息, 默认NO(不打印log).
        value 设置为YES,umeng SDK 会输出log信息可供调试参考. 除非特殊需要，否则发布产品时需改回NO.
...
/** 结构化事件
 使用前，请先到友盟App管理后台的设置->编辑自定义事件 中添加相应的事件ID。
         ...
         keyPath 字符串数组代表的结构化事件路径，其长度最大为8，不能使用unicode 48以内的字符，keyPath[0]必须在网站注册事件ID.
