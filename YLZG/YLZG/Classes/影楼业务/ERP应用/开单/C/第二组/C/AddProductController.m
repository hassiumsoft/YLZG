...
- (void)setupMomTableView
{
    ...
    searchBar.placeholder = @"产品关键字";
    [self.view addSubview:searchBar];
    self.searchBar = searchBar;
    
    self.definesPresentationContext = YES;
    
    self.momTableV = [[UITableView alloc]initWithFrame:CGRectMake(0, 50, SCREEN_WIDTH * 0.34, self.view.height-50)];
...
- (void)addProduct:(AllProductList *)model
{
    ...
    if ([self.delegate respondsToSelector:@selector(addProductModel:)]) {
        [self.delegate addProductModel:model];
    }
}
...
{
    if (!_sonTableV) {
        _sonTableV = [[UITableView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH * 0.34, 50, SCREEN_WIDTH * 0.66, self.view.height - 50)];
