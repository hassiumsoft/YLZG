...
//  NewHXDemo
//
//  Created by Chan_Sir on 16/6/27.
//  Copyright © 2016年 陈振超. All rights reserved.
//

#import "WifiNetManager.h"

static WifiNetManager *NetworksManager;

@implementation WifiNetManager

+ (WifiNetManager *)sharedWifiNetManager
{
    if (!NetworksManager)
        NetworksManager = [[WifiNetManager alloc] init];
...
+ (NSNumber *)numberFromBSSID:(NSString *) bssid
{
    ...
    if (sscanf([bssid cStringUsingEncoding: [NSString defaultCStringEncoding]],"%hX:%hX:%hX:%hX:%hX:%hX",&bs_in[0], &bs_in[1], &bs_in[2], &bs_in[3], &bs_in[4], &bs_in[5]) == MAC_LEN)
    {
        for (int x = 0; x < MAC_LEN; x++)
            longmac |= (uint64_t) bs_in[x] << ((MAC_LEN - 1 - x) * 8);
    } else {
...
- (void)scan
{
    //    NSLog(@"Scanning…");
    scanning = true;
    [YLNotificationCenter postNotificationName:@"startedScanning" object:self];
