...
- (void)setupSubViews
{
    self.array = [NSArray arrayWithObjects:@[@"自动登录",@"推送设置"],@[@"黑名单",@"退群时删除会话",@"消息根据服务器时间排序"],@[@"显示视频通话信息",@"设置视频通话码率"],@[@"退出登录"], nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            cell.textLabel.text = self.array[indexPath.section][indexPath.row];
            [cell setAccessoryType:UITableViewCellAccessoryNone];
            [cell addSubview:self.callTimeSwitch];
            return cell;
        } else {
            // 设置视频通话码率
            NormalTableCell *cell = [NormalTableCell sharedNormalTableCell:tableView];
...
- (void)showCallInfoChanged:(UISwitch *)control
{
    ...
    [userDefaults setObject:[NSNumber numberWithBool:control.isOn] forKey:HXShowCallInfo];
...
- (UISwitch *)sortMethodSwitch
{
    if (!_sortMethodSwitch) {
        ...
        [_sortMethodSwitch addTarget:self action:@selector(showCallInfoChanged:) forControlEvents:UIControlEventValueChanged];
    }
...
- (UISwitch *)callTimeSwitch
{
    if (!_callTimeSwitch)
    {
        _callTimeSwitch = [[UISwitch alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 65, 5, 40, 33)];
        _callTimeSwitch.on = [[[NSUserDefaults standardUserDefaults] objectForKey:@"showCallInfo"] boolValue];
        [_callTimeSwitch addTarget:self action:@selector(showCallInfoChanged:) forControlEvents:UIControlEventValueChanged];
    }
    return _callTimeSwitch;
}
