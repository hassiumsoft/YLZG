...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    if (self.model.isSender) {
        ...
        // 对方
        KGLog(@"IMessageModel = %@",model.message.ext);
        // iOS端有扩展
        NSString *avatarURLPath = [[model.message.ext objectForKey:@"avatarURLPath"] description];
        NSString *nickname = [[model.message.ext objectForKey:@"nickname"] description];
        
        NSString *userName = model.message.from;
