                           ...
                           conversationType:(EMConversationType)conversationType
{
    ...
    if (self) {
        ...
        if (conversationType == EMConversationTypeChat) {
            _chatType = EMChatTypeChat;
        }else{
            _chatType = EMChatTypeGroupChat;
        }
...
- (void)viewDidLoad {
    ...
    [[EaseChatBarMoreView appearance] setMoreViewBackgroundColor:ToolBarColor];
...
- (void)setupEmotion
{
    if ([self.dataSource respondsToSelector:@selector(emotionFormessageViewController:)]) {
        ...
        for (NSString *name in [EaseEmoji allEmoji]) {
            EaseEmotion *emotion = [[EaseEmotion alloc] initWithName:@"上课看看" emotionId:name emotionThumbnail:name emotionOriginal:name emotionOriginalURL:@"" emotionType:EMEmotionDefault];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[IMChatManager sharedManager] setIsShowingimagePicker:NO];
...
- (void)userDidJoinChatroom:(EMChatroom *)aChatroom user:(NSString *)aUsername
{
    [[YLZGDataManager sharedManager] getOneStudioByUserName:aUsername Block:^(ContactersModel *model) {
        NSString *message = [NSString stringWithFormat:@"%@加入群聊",model.nickname];
                            ...
                            username:(NSString *)aUsername
{
    ...
    [[YLZGDataManager sharedManager] getOneStudioByUserName:aUsername Block:^(ContactersModel *model) {
        [self showWarningTips:[NSString stringWithFormat:@"%@离开群聊",model.nickname]];
    }];
}
...
- (void)_locationMessageCellSelected:(id<IMessageModel>)model
{
    ...
    SendLocaltionController *location = [[SendLocaltionController alloc]initWithLocation:CLLocationCoordinate2DMake(model.latitude, model.longitude)];
    location.title = @"查看位置";
                     ...
                     append:(BOOL)isAppend
{
    ...
    dispatch_async(_messageQueue, ^{
        __block NSArray *moreMessages = [NSArray array];

        [weakSelf.conversation loadMessagesStartFromId:messageId count:(int)count searchDirection:EMMessageSearchDirectionUp completion:^(NSArray *aMessages, EMError *aError) {
            moreMessages = [aMessages copy];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        EaseMessageTimeCell *timeCell = [EaseMessageTimeCell shatedEaseMessageTimeCell:tableView];
        timeCell.time = object;
        return timeCell;
    }else{   // 非时间cell
...
- (void)moreViewPhotoAction:(EaseChatBarMoreView *)moreView
{
    ...
    self.isViewDidAppear = NO;
    [[IMChatManager sharedManager] setIsShowingimagePicker:YES];
}
#pragma mark - 发送相册照片的回调
...
- (void)moreViewLocationAction:(EaseChatBarMoreView *)moreView
{
    ...
    SendLocaltionController *locationController = [[SendLocaltionController alloc] init];
    locationController.SendLocalBlock = ^(SendLocationModel *model){
        KGLog(@"model.address = %@",model.address);
        // 构造位置消息
        EMMessage *message = [IMChatManager sendLocationMessageWithLatitude:model.latitude longitude:model.longitude address:model.address to:_conversation.conversationId messageType:_chatType];
        [self _sendMessage:message];
    };
    locationController.title = @"发送位置";
...
//            NSString *newestAvatar = [message.ext objectForKey:@"avatarURLPath"];
            [self.conversation setExt:message.ext];
...
#pragma mark - 接收到一条CMD消息

- (void)cmdMessagesDidReceive:(NSArray *)aCmdMessages
{
    for (EMMessage *message in aCmdMessages) {
        if ([self.conversation.conversationId isEqualToString:message.conversationId]) {
            // [self showHint:@"收到CMD消息"];
            break;
        }
    }
}
...
#pragma mark - 当手机靠近耳朵时或者离开耳朵时的回调方法
...
- (void)sendLocationMessageLatitude:(double)latitude
                         ...
                         andAddress:(NSString *)address
{
    EMMessage *message = [IMChatManager sendLocationMessageWithLatitude:latitude longitude:longitude address:address to:self.conversation.conversationId messageType:_chatType];
    [self _sendMessage:message];
}
#pragma mark - 发送二进制图片消息
- (void)sendImageMessageWithData:(NSData *)imageData
{
    ...
    EMMessage *message = [IMChatManager sendImageMessageWithImageData:imageData to:self.conversation.conversationId messageType:_chatType];
...
- (void)sendImageMessage:(UIImage *)image
{
    ...
    EMMessage *message = [IMChatManager sendImageMessageWithImage:image to:self.conversation.conversationId messageType:_chatType];
                             ...
                             duration:(NSInteger)duration
{
    ...
    EMMessage *message = [IMChatManager sendVoiceMessageWithLocalPath:localPath duration:duration to:self.conversation.conversationId messageType:_chatType];
    [self _sendMessage:message];
}
#pragma mark - 发送视频消息
- (void)sendVideoMessageWithURL:(NSURL *)url
{
    ...
    EMMessage *message = [IMChatManager sendVideoMessageWithURL:url to:self.conversation.conversationId messageType:_chatType];
