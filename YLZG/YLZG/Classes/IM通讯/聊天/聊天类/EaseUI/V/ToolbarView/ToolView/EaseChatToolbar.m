...
{
    self = [self initWithFrame:frame horizontalPadding:6 verticalPadding:4 inputViewMinHeight:36 inputViewMaxHeight:150 type:EMChatToolbarTypeGroup];
    if (self) {
...
{
    self = [self initWithFrame:frame horizontalPadding:6 verticalPadding:4 inputViewMinHeight:36 inputViewMaxHeight:150 type:type];
    if (self) {
...
- (void)_setupSubviews
{
    ...
    self.backgroundColor = ToolBarColor;
    ...
    _inputTextView.backgroundColor = RGBACOLOR(252, 252, 252, 1);
    _inputTextView.layer.borderColor = [UIColor colorWithWhite:0.7f alpha:0.8f].CGColor;
...
//    [self.recordButton setBackgroundImage:[self imageWithBgColor:self.backgroundColor] forState:UIControlStateNormal];
//    [self.recordButton setBackgroundImage:[self imageWithBgColor:[UIColor lightGrayColor]] forState:UIControlStateHighlighted];
//    self.recordButton.layer.cornerRadius  = 4;
//    self.recordButton.layer.borderColor = RGBACOLOR(44, 44, 44, 1).CGColor;
...
- (void)dealloc
{
    [YLNotificationCenter removeObserver:self name:UIKeyboardWillChangeFrameNotification object:nil];
    _delegate = nil;
...
- (UIView *)faceView
{
    if (_faceView == nil) {
        ...
        _faceView.backgroundColor = ToolBarColor;
...
- (UIView *)moreView
{
    if (_moreView == nil) {
        ...
        _moreView.backgroundColor = ToolBarColor;
