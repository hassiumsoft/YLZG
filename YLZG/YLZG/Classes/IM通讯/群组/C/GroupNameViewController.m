...
- (void)changeGroupName:(UIButton *)sender
{
    ...
    NSString *url = [NSString stringWithFormat:@"http://192.168.0.158/index.php/home/easemob/edit_group_info?uid=%@&id=%@&gid=%@&name=%@",account.userID,self.groupModel.id,self.groupModel.gid,self.textField.text];
...
- (void)changeGroupDsp
{
    ...
    [self.view endEditing:YES];
    ZCAccount *account = [ZCAccountTool account];
    NSString *url = [NSString stringWithFormat:@"http://192.168.0.158/index.php/home/easemob/edit_group_info?uid=%@&id=%@&gid=%@&dsp=%@",account.userID,self.groupModel.id,self.groupModel.gid,self.textField.text];
