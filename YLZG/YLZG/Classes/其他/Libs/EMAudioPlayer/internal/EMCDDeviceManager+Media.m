...
- (void)asyncPlayingWithPath:(NSString *)aFilePath
    ...
    if (![fileManager fileExistsAtPath:wavFilePath]) {
        ...
        if (!covertRet) {
            if (completon) {
                completon([NSError errorWithDomain:@"格式转换失败"
...
- (void)asyncStartRecordingWithFileName:(NSString *)fileName
    ...
    if ([self isRecording]) {
        if (completion) {
            error = [NSError errorWithDomain:@"录音尚未结束"
    ...
    if (!fileName || [fileName length] == 0) {
        error = [NSError errorWithDomain:@"文件路径找不到"
                                    code:-1
                                userInfo:nil];
        completion(error);
        return ;
...
-(void)asyncStopRecordingWithCompletion:(void(^)(NSString *recordPath,
    ...
    if(![self isRecording]){
        if (completion) {
            error = [NSError errorWithDomain:@"录音未开始"
    ...
    if([_recorderEndDate timeIntervalSinceDate:_recorderStartDate] < [EMCDDeviceManager recordMinDuration]){
        if (completion) {
            error = [NSError errorWithDomain:@"录音时间太短"
    ...
    if (isNeedActive) {
        BOOL success = [audioSession setActive:isActive
                                   withOptions:AVAudioSessionSetActiveOptionNotifyOthersOnDeactivation
                                         error:&error];
        if(!success || error){
            error = [NSError errorWithDomain:@"播放器加载失败"
                                        code:-1
                                    userInfo:nil];
            return error;
        }
