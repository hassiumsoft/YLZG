...
{
    QZConditionFilterView *conditionFilter = [[QZConditionFilterView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 44)];
...
- (void)createSubView
{
    ...
    UILabel *middleLine2=[[UILabel alloc] initWithFrame:CGRectMake(_dataSource1Btn.x+_dataSource1Btn.width, 8 , 0.5, 24)];
    ...
    [_dataSource2Btn addTarget:self action:@selector(filterChoseData3:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)prepareUIWithBtn:(UIButton *)btn
{
    ...
    if (btn == _dataSource1Btn) {
        _filterTableView1 = [[QZFilterDataTableView alloc] initWithFrame:CGRectMake(0, 108, self.width, 0)];
        ...
        _filterTableView2 = [[QZFilterDataTableView alloc] initWithFrame:CGRectMake(0, 108, self.width, 0)];
...
{
    if (firstAry != nil) {
        NineCategoryModel *firstModel = [firstAry firstObject];
        [self changeBtn:_dataSource1Btn Text:firstModel.name Font:[UIFont systemFontOfSize:13] ImageName:@"PR_filter_choice"];
...
-(void)choseSort:(NSArray *)sortAry
{
    if (_dataSource1Btn.selected) {
        // 改变btn显示的数据
        NineCategoryModel *firstModel = [sortAry firstObject];
        [self changeBtn:_dataSource1Btn Text:firstModel.name Font:[UIFont systemFontOfSize:13] ImageName:@"PR_filter_choice"];
        // 存储显示的数据
...
{
    NineCategoryModel *firstModel1 = [dataAry1 firstObject];
    NSString *data2Str = [dataAry2 firstObject];
    

    [self changeBtn:_dataSource1Btn Text:firstModel1.name Font:[UIFont systemFontOfSize:13] ImageName:@"PR_filter_choice"];
